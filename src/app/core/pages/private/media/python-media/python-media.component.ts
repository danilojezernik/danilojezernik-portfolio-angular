...
@Component({
  selector: 'app-python-media',
  standalone: true,
  imports: [CommonModule, BreadcrumbAdminComponent, GoBackComponent, HeroTitleComponent],
  templateUrl: './python-media.component.html'
})
export class PythonMediaComponent implements OnInit {

  // Inject MyGalleryService to interact with backend services for media management
  protected _pythonService = inject(PythonService);
  ...
  getAllImages(): void {
    ...
    this._pythonService.getAllPythonImages().pipe(
      catchError(error => {
  ...
  upload(): void {
    if (this.selectedFile) {
      ...
      this._pythonService.uploadPythonImage(this.selectedFile).pipe(
        catchError(error => {
  ...
  deleteImage(filename: string): void {
    ...
    if (yes) {
      this._pythonService.deletePythonImageByName(filename).pipe(
        catchError(error => {
          // Capture and display error message
          this.error = error.message;
          // Return null to handle errors gracefully
