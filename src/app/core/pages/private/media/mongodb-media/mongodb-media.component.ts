...
@Component({
  selector: 'app-mongodb-media',
  standalone: true,
  imports: [CommonModule, BreadcrumbAdminComponent, GoBackComponent, HeroTitleComponent],
  templateUrl: './mongodb-media.component.html'
})
export class MongodbMediaComponent implements OnInit {

  // Inject MyGalleryService to interact with backend services for media management
  protected _mongodbService = inject(MongodbService);
  ...
  getAllImages(): void {
    ...
    this._mongodbService.getAllMongoDbImages().pipe(
      catchError(error => {
  ...
  upload(): void {
    if (this.selectedFile) {
      ...
      this._mongodbService.uploadMongoDbImage(this.selectedFile).pipe(
        catchError(error => {
  ...
  deleteImage(filename: string): void {
    ...
    if (yes) {
      this._mongodbService.deleteMongoDbImageByName(filename).pipe(
        catchError(error => {
          // Capture and display error message
          this.error = error.message;
          // Return null to handle errors gracefully
