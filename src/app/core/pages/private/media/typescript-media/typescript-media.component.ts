...
@Component({
  selector: 'app-typescript-media',
  standalone: true,
  imports: [CommonModule, BreadcrumbAdminComponent, GoBackComponent, HeroTitleComponent],
  templateUrl: './typescript-media.component.html'
})
export class TypescriptMediaComponent implements OnInit {

  // Inject MyGalleryService to interact with backend services for media management
  protected _typeScriptService = inject(TypescriptService);
  ...
  getAllImages(): void {
    ...
    this._typeScriptService.getAllTypescriptImages().pipe(
      catchError(error => {
  ...
  upload(): void {
    if (this.selectedFile) {
      ...
      this._typeScriptService.uploadTypescriptImage(this.selectedFile).pipe(
        catchError(error => {
  ...
  deleteImage(filename: string): void {
    ...
    if (yes) {
      this._typeScriptService.deleteTypescriptImageByName(filename).pipe(
        catchError(error => {
          // Capture and display error message
          this.error = error.message;
          // Return null to handle errors gracefully
