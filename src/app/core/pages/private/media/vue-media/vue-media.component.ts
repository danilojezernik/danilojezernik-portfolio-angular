...
export class VueMediaComponent implements OnInit {
  ...
  getAllImages(): void {
    ...
    this._vueService.getAllVueImages().pipe(
      catchError(error => {
  ...
  upload(): void {
    if (this.selectedFile) {
      ...
      this._vueService.uploadVueImage(this.selectedFile).pipe(
        catchError(error => {
  ...
  deleteImage(filename: string): void {
    ...
    if (yes) {
      this._vueService.deleteVueImageByName(filename).pipe(
        catchError(error => {
          // Capture and display error message
          this.error = error.message;
          // Return null to handle errors gracefully
