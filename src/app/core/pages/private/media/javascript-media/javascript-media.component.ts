...
@Component({
  selector: 'app-javascript-media',
  standalone: true,
  imports: [CommonModule, BreadcrumbAdminComponent, GoBackComponent, HeroTitleComponent],
  templateUrl: './javascript-media.component.html'
})
export class JavascriptMediaComponent implements OnInit {

  // Inject MyGalleryService to interact with backend services for media management
  protected _javaScriptService = inject(JavascriptService);
  ...
  getAllImages(): void {
    ...
    this._javaScriptService.getAllJavascriptImages().pipe(
      catchError(error => {
  ...
  upload(): void {
    if (this.selectedFile) {
      ...
      this._javaScriptService.uploadJavascriptImage(this.selectedFile).pipe(
        catchError(error => {
  ...
  deleteImage(filename: string): void {
    ...
    if (yes) {
      this._javaScriptService.deleteJavascriptImageByName(filename).pipe(
        catchError(error => {
          // Capture and display error message
          this.error = error.message;
          // Return null to handle errors gracefully
